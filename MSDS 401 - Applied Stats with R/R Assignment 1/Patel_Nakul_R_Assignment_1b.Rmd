...
y <- f_trig(x)
x_y_min <- c(round(x[which.min(y)], 3), round(y[which.min(y)], 3))
x_y_min
...
ggplot(data = NULL, aes(x, y)) + 
  ...
  labs(x = 'x', y = 'y', title = 'y = sin(x/2) - cos(x/2)') + 
  geom_point(aes(x = x_y_min[1], y = x_y_min[2]), color = 'red') + 
  geom_text(aes(x = x_y_min[1], y = x_y_min[2]), nudge_y = .09, 
                label = '( -1.571, -1.414 )')
...
ggplot(data = NULL, aes(x, y1)) + 
  geom_line(color = 'blue') + 
  geom_line(aes(x, y2), color = 'green') + 
  geom_point(aes(x = 0, y = 0), color = 'red') + 
  geom_text(aes(x=0, y =0), nudge_y = .12, label = '(0, 0)')
...
mean_values
length(trees[trees$Volume > mean_values[3],3])
...
hist(x = cross_area, breaks = 10, col = 'light green', 
     xlab = 'Cross Section Area', ylab = 'Frequency', main = 'Frequency of Cross Section Areas of
     Tress')
...
par(mfrow = c(2, 2))
hist(x, breaks = 10, col = "light green")
boxplot(x,horizontal = TRUE, col= "light green")
hist(y, breaks = 10, col = "blue")
...
par(mfrow = c(1, 2))
qqnorm(x, col = 'light green', main = 'QQ Plot for X (Normal Distribution)')
qqline(x)
qqnorm(y, col = 'blue', main = 'QQ Plot for T (Heavy Distribution)')
qqline(y)
